...
const Navbar = ({ navOpen }: { navOpen: boolean }) => {
  ...
  window.addEventListener("resize", initActiveBox); // Add event listener to handle window resize
  const activeCurrentLink = (e: React.MouseEvent<HTMLAnchorElement>) => {
    if (!activeBox.current || !lastActiveLink.current) return; // <-- early return
